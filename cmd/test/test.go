package main

import (
	"encoding/json"
	"flag"
	"fmt"
	"go-pinterest/config"
	"go-pinterest/model"
)

var (
	//SessionKey    = ""
	//Csrftoken     = "1d7df6fbd5e80a9b140abd4196ccf0d6"
	//maxRelated    = flag.Int("max_related_pin", 50, "max items per query related")
	conf = flag.String("conf", "/home/tamnb/projects/src/github.com/nguyenbatam/crawl_pinterest_mysql/investing_economic_calender.toml", "config run file *.toml")
	c    = config.CrawlConfig{}
)

func main() {
	a:=model.EventQuery{From: "2023-05-13",To: "2023-05-14", }
	t,_:=json.Marshal(a)
	fmt.Println(string(t))
}
